...
// Copyright (c) 2017 Adam Schackart
// -----------------------------------------------------------------------------
#ifndef PAR_SHAPES_H
#define PAR_SHAPES_H
...
#if defined(_MSC_VER)
    typedef unsigned __int8 par_uint8_t;
    typedef signed __int8 par_int8_t;
    typedef unsigned __int16 par_uint16_t;
    typedef signed __int16 par_int16_t;
    typedef unsigned __int32 par_uint32_t;
    typedef signed __int32 par_int32_t;
    typedef unsigned __int64 par_uint64_t;
    typedef signed __int64 par_int64_t;
#else
    #include <stdint.h>

    typedef uint8_t par_uint8_t;
    typedef int8_t par_int8_t;
    typedef uint16_t par_uint16_t;
    typedef int16_t par_int16_t;
    typedef uint32_t par_uint32_t;
    typedef int32_t par_int32_t;
    typedef uint64_t par_uint64_t;
    typedef int64_t par_int64_t;
#endif

#ifndef PAR_SHAPES_T
#define PAR_SHAPES_T par_uint16_t
#endif
...
typedef void (*par_shapes_fn)(float const* uv, float* xyz, void* data);
...
par_shapes_mesh*
par_shapes_create_lsystem(char const* program, int slices, int maxdepth);
...
par_shapes_mesh*
par_shapes_clone(par_shapes_mesh const* mesh, par_shapes_mesh* target);
...
struct par__osn_context;

static int par__simplex_noise(par_int64_t seed, struct par__osn_context** ctx);
static void par__simplex_noise_free(struct par__osn_context* ctx);
static double par__simplex_noise2(struct par__osn_context* ctx, double x, double y);
...
static void
par_shapes__transform3(float* p, float const* x, float const* y, float const* z)
{
    const float px = par_shapes__dot3(p, x);
    const float py = par_shapes__dot3(p, y);
    const float pz = par_shapes__dot3(p, z);
...
{
    const float x = b[0] * t + a[0] * (1 - t);
    const float y = b[1] * t + a[1] * (1 - t);
    const float z = b[2] * t + a[2] * (1 - t);
...
{
    const float lsqr = sqrt(v[0]*v[0] + v[1]*v[1] + v[2]*v[2]);
    if (lsqr > 0)
    {
...
par_shapes_mesh* par_shapes_create_klein_bottle(int slices, int stacks)
{
    ...
    for (int stack = 0; stack < stacks; stack++, face += 0)
...
void par_shapes_export(par_shapes_mesh const* mesh, char const* filename)
{
    ...
    else if (norms)
    {
        ...
        for (int nface = 0; nface < mesh->ntriangles; nface++)
        {
            const int a = 1 + *indices++;
            const int b = 1 + *indices++;
            const int c = 1 + *indices++;
...
{
    const float phi   = uv[0] * 1 * PAR_PI;
    const float theta = uv[1] * 2 * PAR_PI;
...
{
    const float phi   = uv[0] * PAR_PI;
    const float theta = uv[1] * PAR_PI;
...
{
    const float major = 1;
    const float minor = *((float*) userdata);
    const float theta = uv[0] * 2 * PAR_PI;
    const float phi = uv[1] * 2 * PAR_PI;
    const float beta = major + minor * cosf(phi);
...
void par_shapes_rotate(par_shapes_mesh* mesh, float radians, float const* axis)
{
    ...
    {
        const float x = col0[0] * p[0] + col1[0] * p[1] + col2[0] * p[2];
        const float y = col0[1] * p[0] + col1[1] * p[1] + col2[1] * p[2];
        const float z = col0[2] * p[0] + col1[2] * p[1] + col2[2] * p[2];
...
par_shapes_mesh* par_shapes_create_tetrahedron(void)
{
    ...
    {
         0.000f, 1.333f,  0.000f,
         0.943f, 0.000f,  0.000f,
        -0.471f, 0.000f,  0.816f,
        -0.471f, 0.000f, -0.816f,
    };
        ...
        par_shapes_mesh* turtle, float const* pos, float const* scale)
{
    ...
    {
        float* pnt = m->points + p * 3;

        pnt[0] *= scale[0];
        pnt[1] *= scale[1];
        pnt[2] *= scale[2];

        par_shapes__transform3(pnt, turtle->points + 0,
                                    turtle->points + 3,
                                    turtle->points + 6);
        pnt[0] += pos[0];
        pnt[1] += pos[1];
...
static void
par_shapes__sort_points(par_shapes_mesh* mesh, int gridsize, PAR_SHAPES_T* sortmap)
{
    ...
    for (int p = 0; p < mesh->npoints; p++, pt += 3)
    {
        ...
        for (int k = minp[2]; k <= maxp[2]; k++)
        {
            ...
            if (binvalue > 0)
            {
                if (nbins == 8)
                {
                    PAR_PRINT("Epsilon value is too large.");
                    break;
                }
        ...
        for (int b = 0; b < nbins; b++)
        {
            ...
            while (1)
            {
                ...
                if (nextbinindex != binindex) break;
            }
        }
    }
...
par_shapes_mesh* par_shapes_weld(par_shapes_mesh const* mesh,
                        float epsilon, PAR_SHAPES_T* weldmap)
{
...
#define PAR__STRETCH_CONSTANT_2D (-0.211324865405187) // (1 / sqrt(2 + 1) - 1 ) / 2;
#define PAR__SQUISH_CONSTANT_2D (0.366025403784439) // (sqrt(2 + 1) -1) / 2;
#define PAR__NORM_CONSTANT_2D (47.0)

struct par__osn_context
{
    par_int16_t* perm;
    par_int16_t* permGradIndex3D;
};

#define PAR__ARRAYSIZE(x) (sizeof((x)) / sizeof((x)[0]))

/* Gradients for 2D. They approximate the directions to the
 * vertices of an octagon from the center.
 */
static const signed char par__gradients2D[] = {
    5, 2, 2, 5, -5, 2, -2, 5, 5, -2, 2, -5, -5, -2, -2, -5,
};

/* Gradients for 3D. They approximate the directions to the vertices of
 * a rhombicuboctahedron from the center, skewed so that the triangular
 * and square facets can be inscribed inside circles of the same radius.
 */
static const signed char par__gradients3D[] = {
...
static double par__extrapolate2(
    struct par__osn_context* ctx, int xsb, int ysb, double dx, double dy)
{
    par_int16_t* perm = ctx->perm;

    const int index = perm[(perm[xsb & 0xFF] + ysb) & 0xFF] & 0x0E;
    return par__gradients2D[index] * dx + par__gradients2D[index+1] * dy;
}

static inline int par__fastfloor(double x)
{
    const int xi = (const int)x;
    return x < xi ? xi - 1 : xi;
}

static int par__allocate_perm(struct par__osn_context* ctx, int nperm, int ngrad)
{
    PAR_FREE(ctx->perm);
    PAR_FREE(ctx->permGradIndex3D);

    ctx->perm = PAR_MALLOC(par_int16_t, nperm);
    if (!ctx->perm)
    {
        return -ENOMEM;
    }

    ctx->permGradIndex3D = PAR_MALLOC(par_int16_t, ngrad);
...
static int par__simplex_noise(par_int64_t seed, struct par__osn_context** ctx)
{
    par_int16_t source[256];
    int i, rc;

    par_int16_t* perm;
    par_int16_t* permGradIndex3D;

    *ctx = PAR_MALLOC(struct par__osn_context, 1);
    if (!(*ctx))
    {
        return -ENOMEM;
    }
    ...
    rc = par__allocate_perm(*ctx, 256, 256);
    if (rc)
    {
        PAR_FREE(*ctx);
        return rc;
    ...
    for (i = 255; i >= 0; i--)
    {
        ...
        permGradIndex3D[i] = (short)
            ((perm[i] % (PAR__ARRAYSIZE(par__gradients3D) / 3)) * 3);
...
static double par__simplex_noise2(struct par__osn_context* ctx, double x, double y)
{
    ...
    double stretchOffset = (x + y) * PAR__STRETCH_CONSTANT_2D;
    double xs = x + stretchOffset;
    double ys = y + stretchOffset;

    // Floor to get grid coordinates of rhombus (stretched square) super-cell origin.
    int xsb = par__fastfloor(xs);
    int ysb = par__fastfloor(ys);

    // Skew out to get actual coordinates of rhombus origin. We'll need these later.
    double squishOffset = (xsb + ysb) * PAR__SQUISH_CONSTANT_2D;
    ...
    // Contribution (1,0)
    double dx1 = dx0 - 1 - PAR__SQUISH_CONSTANT_2D;
    double dy1 = dy0 - 0 - PAR__SQUISH_CONSTANT_2D;
    double attn1 = 2 - dx1 * dx1 - dy1 * dy1;
    if (attn1 > 0) {
        attn1 *= attn1;
        value += attn1 * attn1 * par__extrapolate2(ctx, xsb + 1, ysb + 0, dx1, dy1);
    }

    // Contribution (0,1)
    double dx2 = dx0 - 0 - PAR__SQUISH_CONSTANT_2D;
    double dy2 = dy0 - 1 - PAR__SQUISH_CONSTANT_2D;
    double attn2 = 2 - dx2 * dx2 - dy2 * dy2;
    if (attn2 > 0) {
        attn2 *= attn2;
        value += attn2 * attn2 * par__extrapolate2(ctx, xsb + 0, ysb + 1, dx2, dy2);
    }
    ...
    if (inSum <= 1) {  // We're inside the triangle (2-Simplex) at (0,0)
        ...
        if (zins > xins || zins > yins) {
            ...
            xsv_ext = xsb + 1;
            ysv_ext = ysb + 1;
            dx_ext = dx0 - 1 - 2 * PAR__SQUISH_CONSTANT_2D;
            dy_ext = dy0 - 1 - 2 * PAR__SQUISH_CONSTANT_2D;
        }
        ...
        if (zins < xins || zins < yins) {
            if (xins > yins) {
                xsv_ext = xsb + 2;
                ysv_ext = ysb + 0;
                dx_ext = dx0 - 2 - 2 * PAR__SQUISH_CONSTANT_2D;
                dy_ext = dy0 + 0 - 2 * PAR__SQUISH_CONSTANT_2D;
            } else {
                xsv_ext = xsb + 0;
                ysv_ext = ysb + 2;
                dx_ext = dx0 + 0 - 2 * PAR__SQUISH_CONSTANT_2D;
                dy_ext = dy0 - 2 - 2 * PAR__SQUISH_CONSTANT_2D;
            }
        } else {  //(1,0) and (0,1) are the closest two vertices.
            dx_ext = dx0;
            dy_ext = dy0;
        ...
        xsb += 1;
        ysb += 1;
        dx0 = dx0 - 1 - 2 * PAR__SQUISH_CONSTANT_2D;
        dy0 = dy0 - 1 - 2 * PAR__SQUISH_CONSTANT_2D;
    }
    ...
    {
        attn0 *= attn0;
        value += attn0 * attn0 * par__extrapolate2(ctx, xsb, ysb, dx0, dy0);
    }
    ...
    if (attn_ext > 0)
    {
        ...
        value += attn_ext * attn_ext * par__extrapolate2( ctx,
                            xsv_ext, ysv_ext, dx_ext, dy_ext);
    }

    return value / PAR__NORM_CONSTANT_2D;
}
